...
def compute_scores_and_write_to_csv(
    target_filepattern, prediction_filepattern, output_filename, scorer, aggregator, delimiter="\n"
):
...
def _compute_scores(target_filenames, prediction_filenames, scorer, delimiter):
    ...
    scores = []
    for target_filename, prediction_filename in zip(sorted(target_filenames), sorted(prediction_filenames)):
        ...
        for target_rec, prediction_rec in zip_longest(targets, preds):
            if target_rec is None or prediction_rec is None:
                raise ValueError(
                    ...
                    f"prediction files. Mismatch between files: {target_filename}, {prediction_filename}."
                )
...
def _write_aggregates_to_csv(output_filename, aggregates):
    ...
    with _open(output_filename, "w") as output_file:
        ...
        for score_type, aggregate in sorted(aggregates.items()):
            output_file.write(
                f"{score_type}-R,{aggregate.low.recall:f},{aggregate.mid.recall:f},{aggregate.high.recall:f}\n"
            )
            output_file.write(
                f"{score_type}-P,{aggregate.low.precision:f},{aggregate.mid.precision:f},{aggregate.high.precision:f}\n"
            )
            output_file.write(
                f"{score_type}-F,{aggregate.low.fmeasure:f},{aggregate.mid.fmeasure:f},{aggregate.high.fmeasure:f}\n"
            )
...
def _write_scores_to_csv(output_filename, scores):
    ...
    with _open(output_filename, "w") as out_file:
        ...
        for rouge_type in rouge_types:
            out_file.write(f",{rouge_type}-P,{rouge_type}-R,{rouge_type}-F")
        out_file.write("\n")
        for i, result in enumerate(scores):
            out_file.write("%d" % i)
            for rouge_type in rouge_types:
                out_file.write(
                    f",{result[rouge_type].precision:f},{result[rouge_type].recall:f},{result[rouge_type].fmeasure:f}"
                )
            out_file.write("\n")
    logging.info("Finished writing results.")
